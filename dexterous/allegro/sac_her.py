...
class SACAgent(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        # HER configuration
        her_config = config.get("her_config", {})
        if her_config and her_config.get("enabled", False):
            self.her = {
                "goal_key": her_config.get("goal_key", "goal_quat"),
                "object_key": her_config.get("object_key", "object_quat"), 
                "tolerance": her_config.get("tolerance", 0.1),
                "enabled": True,
                "buffer_ratio": her_config.get("buffer_ratio", 3.0),
            }
            ...
            # Increase replay buffer size for HER transitions
            expanded_buffer_size = int(self.replay_buffer_size * self.her["buffer_ratio"])
        else:
            self.her = None
            self.episode_transitions = {}
            self.her_transitions_count = 0
            expanded_buffer_size = self.replay_buffer_size
    ...
    def play_steps(self, random_exploration = False):
        """
        modify to do random_exploration with goal masking instead
        """
        ...
        for s in range(self.num_steps_per_episode):
            ...
            if self.her and self.her["enabled"]:
                self._store_episode_transitions(obs, action, rewards, next_obs_processed, dones, infos)
    ...
    def _store_episode_transitions(self, obs, action, rewards, next_obs, dones, infos):
        """Store episode data for HER processing."""
        obs_dict = infos.get("observations", {}) if infos else {}
        ...
        if goal is not None and object_state is not None:
            for env_idx in range(dones.shape[0]):
                ...
                step_data = {
                    "observations": obs[env_idx].clone(),
                    "actions": action[env_idx].clone(),
                    "rewards": rewards[env_idx].clone(),
                    "next_observations": next_obs[env_idx].clone(),
    ...
    def _process_completed_episodes(self, dones):
        ...
        for env_idx in range(dones.shape[0]):
            if dones[env_idx] and env_idx in self.episode_transitions:
                ...
                if len(episode) > 1:
                    ...
                    # Add each HER transition to replay buffer
                    for her_data in her_transitions:
                        self.replay_buffer.add(
                            her_data["observations"].unsqueeze(0),
                            her_data["actions"].unsqueeze(0), 
                            her_data["rewards"].unsqueeze(0).unsqueeze(1),
                            her_data["next_observations"].unsqueeze(0),
                            torch.zeros(1, 1, dtype=torch.bool, device=self._device)  # not done for HER
                        )
        ...
        if her_count > 0:
            print(f"Added {her_count} HER transitions to replay buffer")
    ...
    def _create_her_transitions(self, episode):
        ...
        for i in range(len(episode) - 1):
            ...
            if original_goal is not None:
                ...
                next_obs_with_original_goal = self._relabel_observation(step["next_observations"], original_goal)
                ...
                her_transitions.append({
                    ...
                    "next_observations": next_obs_with_original_goal,
            ...
            next_obs_with_next_goal = self._relabel_observation(step["next_observations"], next_state)
            success_reward = torch.tensor(1.0, device=self._device, dtype=step["rewards"].dtype)
            
            her_transitions.append({
                "observations": obs_with_next_goal,
                "actions": step["actions"],
                "rewards": success_reward,
                "next_observations": next_obs_with_next_goal,
            ...
            next_obs_with_final_goal = self._relabel_observation(step["next_observations"], final_state)
            ...
            her_transitions.append({
                ...
                "next_observations": next_obs_with_final_goal,
                "her_type": "final_goal"
            })
