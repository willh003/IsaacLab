...
class SACUTDAgent(BaseAlgorithm):
    """SAC Agent with configurable Update-to-Data (UTD) ratio.
    
    Supports UTD ratios both < 1 and > 1:
    - UTD < 1: Updates happen less frequently than data collection
    - UTD > 1: Multiple updates per data collection step  
    - UTD = 1: Standard SAC behavior (1 update per step)
    """
    ...
    def __init__(self, base_name, params):
        ...
        # UTD configuration
        self.utd_ratio = config.get("utd_ratio", 1.0)
        print(f"UTD ratio: {self.utd_ratio}")
        
        # UTD tracking variables
        self.update_counter = 0.0  # Fractional counter for UTD < 1
        self.pending_updates = 0   # Accumulated updates for UTD > 1
    ...
    def should_update_this_step(self):
        ...
        if self.utd_ratio >= 1.0:
            # UTD >= 1: Accumulate fractional updates
            self.pending_updates += self.utd_ratio
            num_updates = int(self.pending_updates)
            self.pending_updates -= num_updates
            return num_updates
        else:
            # UTD < 1: Update probabilistically
            self.update_counter += self.utd_ratio
    ...
    def update(self, step):
        """Perform updates based on UTD ratio."""
        ...
        if actor_losses:
            ...
            avg_alpha = torch.stack(alphas).mean()
    ...
    def play_steps(self, random_exploration = False):
        """
        Modified play_steps with UTD support
        """
        ...
        for s in range(self.num_steps_per_episode):
            ...
            if not random_exploration:
                ...
                # Only extract stats if we actually performed updates
                if actor_loss_info is not None:
