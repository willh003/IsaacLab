...
def inspect_scene_prims(base_prim_path="/World"):
    ...
    # Check for lights
    light_prims = sim_utils.findself.max_epi_matching_prims("/World/Light")
...
class ReorientationEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: LeapHandEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # track timeout resets (only update goals on timeout resets)
        self.timeout_reset_buf = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ...
        self.episode_lengths = torch.ones((self.num_envs,), dtype=torch.int32, device=self.device) * self.max_episode_length
    ...
    def _update_continuous_z_rotation(self, goal_env_ids):        
        ...
        angle_axis = torch.rand(self.num_envs, device=self.device) * 2 * math.pi
        
        quat = quat_from_angle_axis(angle_axis, self.z_unit_tensor)
        #add_rot = quat_from_angle_axis(angle_axis, self.z_unit_tensor)

        self.goal_rot[goal_env_ids] = quat[goal_env_ids]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self.extras["log"]["avg_episode_length_s"] = (self.episode_lengths.float() * self.cfg.sim.dt * self.cfg.decimation).mean()
        self.extras["log"]["min_episode_length_s"] = (self.episode_lengths.float() * self.cfg.sim.dt * self.cfg.decimation).min()
        self.extras["log"]["max_episode_length_s"] = (self.episode_lengths.float() * self.cfg.sim.dt * self.cfg.decimation).max()

        if self.cfg.enable_adr:
            adr_criteria = ((self.consecutive_successes / self.cfg.z_rotation_steps) / (self.episode_lengths.float().mean() * self.cfg.sim.dt * self.cfg.decimation)).float().mean()
        ...
        # Note: Goal resets are disabled - goals only update on timeout resets, not when reached
        # This ensures the agent must work with the same goal throughout the episode

        # Always update goal markers visualization
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        time_out = self.episode_length_buf >= self.episode_lengths - 1
        ...
        # Track which resets are due to timeout (for goal updates)
        self.timeout_reset_buf = time_out.clone()
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.cfg.enable_adr:
            adr_criteria = ((self.consecutive_successes.float().mean() / self.cfg.z_rotation_steps) / (self.episode_lengths.float().mean() * self.cfg.sim.dt * self.cfg.decimation)).float().mean()
        ...
        if self.cfg.enable_adr:
            ...
            if z_rot > 0:
                z_rot_noise = sample_uniform(-.1, .1, (len(env_ids_list),), device=self.device)
        ...
        # Only update goal rotations if this reset is due to timeout
        if env_ids is not None:
            timeout_env_ids = torch.where(self.timeout_reset_buf[env_ids])[0]
            if len(timeout_env_ids) > 0:
                actual_timeout_env_ids = env_ids[timeout_env_ids]
                self._update_continuous_z_rotation(actual_timeout_env_ids)
